...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode, debug=False):
        ...
        for aid in db.anns.keys():
            ...
            if self.mode == 'train' and count % train_select_step != 0:
                continue
            if self.mode == 'test' and count % 4 != 0:
                continue
    ...
    def __getitem__(self, idx):
        ...
        for ht in ["right", "left"]:
            ...
            if mano_param is None:
                ...
                mano_mesh_imgs.append(torch.zeros((self.vert_num, 2)))
            ...
            if cfg.crop2full:
                ...
                cat_img_ = torch.from_numpy(cat_cam.copy()).float().permute(1, 0).unsqueeze(0)
                mano_calib_ = torch.from_numpy(calib_).float()
                cat_img_ = perspective(cat_img_, mano_calib_.unsqueeze(0))
                cat_img_ = cat_img_.permute(0, 2, 1).squeeze(0).numpy() 
                mano_mesh_imgs.append(torch.from_numpy(cat_img_[:self.vert_num, :2]))
            else:
                mano_mesh_imgs.append(torch.from_numpy(cat_img[:self.vert_num, :2]))
        ...
        trans_rot2crop = gen_trans_from_patch_cv(center[0], center[1], bbox[2], bbox[3], int(cfg.input_img_shape[1]), int(cfg.input_img_shape[0]), scale, 0)
        trans_rot2crop = torch.tensor(trans_rot2crop).to(torch.float64)
        ...
        # if do_flip == 1:
        #     mask = mask[:, ::-1, :]
        # mask = cv2.warpAffine(mask, trans, (int(cfg.input_img_shape[1]), int(cfg.input_img_shape[0])), flags=cv2.INTER_LINEAR)
        ...
        if self.debug:
            ...
            for jidx, jproj in enumerate(mano_mesh_imgs):
                # if jv == 1:
                #print(jproj)
    ...
    def eval_iou(self, ious, logger):
        # gt_masks/pred_masks:[n, 1, H, W]
        eval_type = "iou"
        iou_sh = []
        iou_ih = []
        tot_err = []
        ...
        for n in range(sample_num):
            ...
            if gt_hand_type == 'right' or gt_hand_type == 'left':
                iou_sh.append(ious[n])
            else:
                iou_ih.append(ious[n])
            tot_err.append(ious[n])

        logger.info('%s for all hand sequences: %.2f' % (eval_type, np.mean(tot_err)))

        logger.info('%s for single hand sequences: %.2f' % (eval_type, np.mean(iou_sh)))

        logger.info('%s for interacting hand sequences: %.2f' % (eval_type, np.mean(iou_ih)))
