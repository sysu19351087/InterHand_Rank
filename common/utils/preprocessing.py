...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type):
    ...
    if do_flip:
        bb_c_x = original_img_shape[1] - bb_c_x - 1
    center = np.array([bb_c_x, bb_c_y]).astype(float)
    bbox_size = estimate_focal_length(bb_height, bb_width)
    b_scale = bbox_size * scale
    ...
    for i in range(joint_num):
        ...
        joint_valid[i] = joint_valid[i] * (joint_coord[i, 0] >= 0) * (joint_coord[i, 0] < cfg.input_img_shape[1]) * (
                    joint_coord[i, 1] >= 0) * (joint_coord[i, 1] < cfg.input_img_shape[0])

    return img, joint_coord, joint_valid, hand_type, trans, inv_trans, center, b_scale, rot, do_flip, scale, color_scale
...
def gen_ori2rotori_trans(c_x, c_y, src_width, src_height, scale, rot, inv=False):
    # augment size with scale
    src_w = src_width * scale
    src_h = src_height * scale
    src_center = np.array([c_x, c_y], dtype=np.float32)
    ...
    dst_w = src_w
    dst_h = src_h
    dst_center = np.array([c_x, c_y], dtype=np.float32)
    dst_downdir = np.array([0, dst_h * 0.5], dtype=np.float32)
    dst_rightdir = np.array([dst_w * 0.5, 0], dtype=np.float32)
...
def trans_poinst2d(pt_2d, trans):
    # pt_2d:[nv, 2]
    # trans:[2, 3]
    nv = pt_2d.shape[0]
    ones = np.ones([nv, 1])
    src_pt = np.concatenate([pt_2d, ones], -1).T   # [3, nv]
    dst_pt = np.dot(trans, src_pt)
    return dst_pt[0:2, :].T     # [nv, 2]
...
def estimate_translation_np(S, joints_2d, joints_conf, focal_length, img_size):
    ...
    Input:
        S: (21, 3) 3D joint locations
        joints: (21, 3) 2D joint locations and confidence
    Returns:
...
def estimate_translation(S, joints_2d, focal_length, img_size):
    """Find camera translation that brings 3D joints S closest to 2D the corresponding joints_2d.
    Input:
        S: (B, 21, 3) 3D joint locations
        joints: (B, 21, 3) 2D joint locations and confidence
    Returns:
    ...
    S = S.cpu().numpy()    # [bs, 21, 3]
    joints_2d = joints_2d.cpu().numpy()   # [bs, 21, 3]
    joints_conf = joints_2d[:, :, -1]                # [bs, 21]
    joints_2d = joints_2d[:, :, :-1]                 # [bs, 21, 2]
    trans = np.zeros((S.shape[0], 3), dtype=np.float32)   # [bs, 3]
    # Find the translation for each example in the batch
    for i in range(S.shape[0]):
        S_i = S[i]      # [21, 3]
        # print(S_i.shape)
        joints_i = joints_2d[i]   # [21, 2]
        conf_i = joints_conf[i]   # [21,]
        #print(joints_i.shape)
