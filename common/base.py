...
class Tester(Base):
    ...
    def set_logger(self, log_dir, log_name='test_logs.txt'):
        while len(self.logger._logger.handlers) > 0:
            h = self.logger._logger.handlers[0]
            self.logger._logger.removeHandler(h)
        del self.logger
        self.logger = colorlogger(log_dir, log_name=log_name)
